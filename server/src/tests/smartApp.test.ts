...
const createLock = async (home_id: string, device_id: string, type: string, label: string, location: string, value: string, battery: number) => {
    const lock = new Lock({home_id: home_id, device_id: device_id, type: type, label: label, location: location, value: value, battery: battery});
...
describe('SmartAppServices behavior:', () => {
    describe("getDevices():", () => {
        ...
        it('Should return list of devices for current home:', async () => {
            ...
            const lock = await createLock(home.id.toString(), 'test-device-id', 'lock', 'testLock', 'test', 'locked', 100)
